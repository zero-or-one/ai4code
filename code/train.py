...
train_ds = MarkdownDataset(train_df_mark, model_name_or_path=config.model_name_or_path, md_max_len=config.md_max_len,
                           total_max_len=config.total_max_len, fts=train_fts)
val_ds = MarkdownDataset(val_df_mark, model_name_or_path=config.model_name_or_path, md_max_len=config.md_max_len,
                         total_max_len=config.total_max_len, fts=val_fts)
train_loader = DataLoader(train_ds, batch_size=config.batch_size, shuffle=True, num_workers=config.n_workers,
...
def train(model, train_loader, val_loader, config.epochs):
    np.random.seed(0)
    ...
    optimizer = AdamW(optimizer_grouped_parameters, lr=config.lr,
                      correct_bias=config.correct_bias)  # To reproduce BertAdam specific behavior set correct_bias=False
    ...
    for e in range(config.epochs):
        ...
        torch.save(model.state_dict(), f"../outputs/{config.name}.bin")
...
cv = StratifiedKFold(n_splits=config.dataset.num_folds)
folds = list(cv.split(np.arange(len(train)), (scores*100).astype(int), anchors))
for fold in fold_index:
'''
model = MarkdownModel(args.model_name_or_path)
model = model.cuda()
model, y_pred = train(model, train_loader, val_loader, epochs=config.epochs)
